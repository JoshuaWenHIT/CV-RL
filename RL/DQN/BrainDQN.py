...
# ACTIONS = 2  # default total available action number for the game: UP and DO NOTHING
ACTIONS = 4


class BrainDQN(nn.Module):
    # empty_frame = np.zeros((128, 72), dtype=np.float32)  # default
    empty_frame = np.zeros((640, 640), dtype=np.float32)
    ...
    def __init__(self, epsilon, mem_size, cuda):
        ...
        self.train = None
        # init replay memory
        self.replay_memory = deque()  # 使用双端队列的数据结构
    ...
    def store_transition(self, o_next, action, reward, terminal):
        ...
        """
        next_state = np.append(self.current_state[1:, :, :], o_next.reshape((1,) + o_next.shape), axis=0)  # default
        # next_state = np.append(self.current_state[1:, :, :, :], o_next.reshape((1,) + o_next.shape), axis=0)  # 3 channels
